...
3. **정사각형 테두리 클리어** — 자석 축 중심 N×N(홀수) **테두리** 완성 시 폭발. **바깥** 블록 제거·**내부** 유지.
...
| 3. 자석 흡착 | 자석 축 방향으로 이동 → 경로상 **가장 먼저** 닿는 **기존 블록 또는 자석 축** 직전에서 스냅 부착. **경계만** 만나고 블록·자석을 못 만나면 **배치 실패(놓을 수 없음)** — 경계에 부착하지 않음 |
| 4. 경계 이탈 | 부착 **성공** 직후, 고정 경계 밖으로 나간 칸이 있으면 **즉시 게임오버** |
| 5. 폭발 판정 | 회전 **전** 좌표. N×N **테두리** 완성 시 **테두리 + N×N 바깥** 제거·점수. **N×N 내부**는 유지 |
...
- **발동 조건:** 해당 N×N **테두리** 칸(체비셰프 거리 = `half`)이 끊김 없이 블록으로 채워짐. 자석 칸 `(0,0)`은 판정·점유 대상 아님.
- **제거 범위** (발동 시):
  - N×N **테두리** 칸 — 제거
  - N×N **바깥** (`max(|x|,|y|) > half`) 점유 칸 — **전부** 제거
  - N×N **내부** (`max(|x|,|y|) < half`) — **유지** (블록·칸 그대로)
- **부분 파괴:** 테두리에 걸친 블록은 **해당 칸만** 제거. 내부에 남는 칸은 offsets 갱신 후 유지.
- **내부 완성(풀 클리어):** 테두리 + 내부까지 모두 채워져도 **제거 범위는 동일**(테두리+바깥만). 내부 블록은 유지. 점수·멀티 보너스용 `isFullClear` 플래그만 별도 (SCRUM-23).
- **동시 클리어:** 여러 N이 동시에 만족하면 제거 칸 **합집합**·`SquareClearedEvent` 크기별 발생·점수 합산.
- **판정 시점:** 부착 직후, **회전 전** 좌표.
...
| SCRUM-20 | 인게임-블록 파괴 판정 & 파괴 | M5 | 테두리 클리어, 테두리+바깥 제거·내부 유지 |
...
| **M5** | 폭발 판정 | JTH | N×N 테두리 검사, 테두리+바깥 제거·내부 유지, 점수 이벤트 | ⬜ |
