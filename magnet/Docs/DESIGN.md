...
2. **Relife:** 위 게임오버이고 세션 점수 ≥ n이며 아직 안 썼으면 Easy 패 3개를 미리 뽑아 `RelifeOfferedEvent`만 쏜다 (`GameOverEvent` 없음). 수락 시 그 패로 이어하기(세션당 1회). 거절은 UI가 `GameOverEvent` 후 재시작.
...
| 연출 | 힌트 클립·클리어 FX·사운드는 `SkinDataSO` 슬롯. 절차는 [`Docs/SKIN.md`](./SKIN.md) |
