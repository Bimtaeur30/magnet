...
| 5. 폭발·재조립 | 회전 **전** 좌표. 완성된 N 중 **가장 안쪽만** 테두리 파괴 → 바깥 칸 부채꼴 목표 재배치 → 다시 판정. 연출 중 입력 잠금. 점수: §4.7 |
...
> **턴(Turn):** 핸드(4후보) 전부 소진·리필 단위. `TurnIndex`·`TurnStarted`/`TurnEnded` 이벤트. `TurnEnded` 시 점수 세션이 턴 중 클리어가 없으면 콤보를 리셋한다. 보드 회전과 혼동하지 말 것.
...
| 기능 | 설명 |
|------|------|
| 해금 | 세션 **누적 점수**가 스킨 `unlockValue` 이상이면 `SkinUnlockCheck`(Score)로 해금 |
| 인벤토리 | 보유·잠금 상태 목록 UI |
