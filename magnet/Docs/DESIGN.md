...
| 한 줄 소개 | 자석 축에 블록이 자동 흡착되고, 정사각형을 완성하면 폭발하며, **배치마다** 보드가 90° 회전하는 블록 퍼즐 |
...
2. **배치마다 90° 회전** — 자석 축 중심으로 보드 전체(부착 블록 포함) 회전. 공간 추론이 배치마다 달라짐.
...
**블록 1개 배치**의 고정 순서 — 이 순서가 게임의 핵심 규칙이다. (보드 90° 회전은 **배치마다** 발생한다.)
...
블록 선택 → x축 배치 → 자석 흡착·부착 (이후 칸 단위)
  ...
  → 자석 축 90° 회전 (보드·칸 동반)
  → 슬롯 소모 (해당 후보 null). 4슬롯 전부 소진 시 턴 종료·리필
```

| 단계 | 설명 |
|------|------|
| 1. 블록 후보 | 화면 하단에 **4개** 무작위 블록 제시. 하나 선택 |
...
| 7. 후보·턴 | 슬롯 1개 소모 시 해당 칸만 비움. **4개 전부 소진** 시 턴 종료 → 4개 리필. 4개 모두 놓을 수 없으면 게임오버 |

> **턴(Turn):** 핸드(4후보) 전부 소진·리필 단위. `TurnIndex`·`TurnStarted`/`TurnEnded` 이벤트로 존재하며, 지금은 플레이 수치(점수·콤보 등)에 연결하지 않는다. 보드 회전과 혼동하지 말 것.
...
- 1개 사용 시 해당 슬롯만 비움(`null`). **4개 전부 소진** 시에만 새 핸드로 `Fill` (턴 종료·시작).
- **게임오버**: 4개 모두 어떤 위치에도 둘 수 없을 때.
...
2. 4개 후보가 **어떤 x에서도** 블록·자석에 흡착될 수 없을 때 (전부 `NoSnapTarget` 등과 동치의 배치 불가).

> **구분:** “경계만 만나 그 자리에 못 놓음”은 **그 시도의 배치 실패**이지, 그 자체로 즉시 게임오버는 아님. 후보 4개 전부가 어떤 x에도 못 붙을 때 게임오버(조건 2).
...
| SCRUM-18 | 인게임-랜덤 블록 생성 | M2 | BlockShapeSO, 4후보 추첨 **로직** |
...
| **M2** | 블록 데이터·공급 | JTH (SCRUM-18) | BlockShapeSO, 4후보 추첨 로직 (UI는 M7·M9) | ⬜ |
