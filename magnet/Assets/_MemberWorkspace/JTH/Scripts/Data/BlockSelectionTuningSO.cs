...
namespace JTH.Scripts.Data
{
    ...
    public sealed class BlockSelectionTuningSO : ScriptableObject
    {
        ...
        [field: SerializeField] public float FillWeight { get; private set; } = 0.4f;

        [field: Tooltip("healthScore에서 dead zone(고립 빈칸 1~3) 성분 가중치")]
        [field: SerializeField] public float DeadZoneWeight { get; private set; } = 0.2f;

        [field: Tooltip("healthScore에서 큰 블록(3x3·1x5) 슬롯 성분 가중치")]
        [field: SerializeField] public float BigSlotWeight { get; private set; } = 0.2f;

        [field: Tooltip("healthScore에서 배치 자유도(테스트 피스 평균 합법 배치 수) 성분 가중치")]
        [field: SerializeField] public float FreedomWeight { get; private set; } = 0.2f;
        ...
        [field: Header("Blame")]
        [field: Tooltip("턴 종료 시 새 dead zone 1개당 blame 증가량 (권장 15~25)")]
        [field: SerializeField] public float BlamePerDeadZone { get; private set; } = 20f;
        ...
        [field: Tooltip("배치 자유도 감소량 1당 blame 증가량 (감소분 × 이 값)")]
        [field: SerializeField] public float BlamePerFreedomDrop { get; private set; } = 0.5f;
        ...
        [field: Tooltip("dead zone 1개당 opportunityScore 감점 (억지 패널티)")]
        [field: SerializeField] public float OpportunityDeadZonePenalty { get; private set; } = 0.15f;
