...
{
    /// <summary>
    /// BoardSession·BlockPlacementService 생성, 부착 후 클리어·회전·Consume까지 턴 Domain을 처리한다.
    /// </summary>
    public sealed class BoardPlacementBootstrap : MonoBehaviour
    {
        ...
        /// <summary>Consume 호출용.</summary>
        public BlockSpawnBootstrap BlockSpawn => _blockSpawnBootstrap;

        public TurnResolutionResult TryConfirmPlacement(IBlockShape shape, Vector2Int startPivot, int slotIndex)
        {
            PlacementResult result = _placementService.TryPlace(shape, startPivot);
            if (!result.Success)
            {
                return TurnResolutionResult.Create(result, ClearDetectionResult.None, boardRotated: false);
            }
            ...
            return TurnResolutionResult.Create(result, clearResult, boardRotated: true);
        }
        ...
        {
            for (int i = 0; i < clearResult.ClearedSquares.Count; i++)
            {
                ClearedSquareInfo square = clearResult.ClearedSquares[i];
