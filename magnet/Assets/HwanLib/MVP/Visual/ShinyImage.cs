...
namespace MVP.Visual
{
    ...
    public class ShinyImage : MonoBehaviour
    {
        ...
        private void Play()
        {
            ...
            _seqHandle = LSequence.Create()
                .Append(LMotion.Create(startX, endX, sweepDuration)
                    .WithEase(ease)
                    .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                    .BindToAnchoredPositionX(highlight))
                .Append(LMotion.Create(0f, 0f, 0f)
                    .WithOnComplete(() => SetX(startX))
                    .Bind(_ => { }))
                .AppendInterval(interval)
                .Run(cfg =>
                {
                    if (ignoreTimeScale) cfg.WithScheduler(MotionScheduler.UpdateIgnoreTimeScale);
                    cfg.WithLoops(-1, LoopType.Restart);
                });
        }
