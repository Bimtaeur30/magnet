...
namespace MVP.Interaction
{
        ...
        IPointerClickHandler, ISelectHandler, IDeselectHandler, ISubmitHandler
    {
        ...
        private readonly CompositeMotionHandle _tweens = new();
        ...
        private void TransitionTo(FeedbackState state)
        {
            ...
            _tweens.Add(LMotion.Create(_canvasGroup.alpha, style.alpha, style.duration)
                .WithEase(style.ease)
                .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                .BindToAlpha(_canvasGroup));
            _tweens.Add(LMotion.Create(transform.localScale.x, style.scale, style.duration)
                .WithEase(style.ease)
                .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
            ...
            foreach (Graphic target in tintTargets)
            {
                ...
                _tweens.Add(LMotion.Create(target.color, style.tint, style.duration)
                    .WithEase(style.ease)
                    .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                    .BindToColor(target));
            }
