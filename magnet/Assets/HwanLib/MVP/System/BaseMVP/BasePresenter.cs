...
namespace MVP.System.BaseMVP
{
    public abstract class BasePresenter : MonoBehaviour
    {
        ...
        // X 버튼·배경 탭 등 닫기 트리거. 클릭 시 Presenter.Close()로 수렴한다.
        [SerializeField] private BaseForm[] closeTriggers;
        ...
        private void BindCloseTriggers()
        {
            if (closeTriggers == null) return;

            foreach (BaseForm form in closeTriggers)
            {
                if (form is not IInteractable interactable) continue;

                FormInteracted handler = _ => Close();
                interactable.OnFormInteracted += handler;
                _closeSubs.Add((interactable, handler));
            }
        }

        private void UnbindCloseTriggers()
        {
