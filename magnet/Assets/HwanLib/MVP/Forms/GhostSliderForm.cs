...
namespace HwanLib.MVP.Forms
{
    ...
    public class GhostSliderForm : AbstractVisualForm
    {
        ...
        private MotionHandle _ghostHandle;
        ...
        protected override void UpdateVisual(UIParam data)
        {
            ...
            if (decreased)
            {
                ...
                if (_ghostHandle.IsActive() == false) PlayNext(); // 재생 중이 아니면 즉시 처리 시작
            }
            else
            {
                // 회복/초기화: 큐와 진행 중 트윈을 모두 비우고 즉시 동기화
                _pending.Clear();
                if (_ghostHandle.IsActive()) _ghostHandle.Cancel();
                _ghostHandle = default;
        ...
        private void PlayNext()
        {
            ...
            _ghostHandle = LMotion.Create(ghostImage.fillAmount, target, ghostDuration)
                .WithDelay(ghostDelay)
                .WithEase(Ease.OutQuad)
                .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                .WithOnComplete(PlayNext)
                .BindToFillAmount(ghostImage);
        }
