...
namespace HwanLib.MVP.Forms
{
    ...
    public class CountBarUpgradeFx : MonoBehaviour
    {
        ...
        private readonly List<MotionHandle> _active = new List<MotionHandle>();
        ...
        {
            if (_form != null) _form.SlotFilled -= OnSlotFilled;
            foreach (MotionHandle handle in _active)
            {
        ...
        private void OnSlotFilled(RectTransform fill)
        {
            ...
            if (delay > 0f) builder.AppendInterval(delay);
            builder.Append(LMotion.Create(0f, 0f, 0f)
                .WithOnComplete(() => SpawnParticle(fill))
                .Bind(_ => { }));
            builder.Append(LMotion.Create(Vector3.one * startScale, Vector3.one, popDuration)
                .WithEase(popEase)
                .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                .BindToLocalScale(fill));
            if (fillImage != null)
            {
                builder.Join(LMotion.Create(0f, 1f, popDuration)
                    .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                    .BindToColorA(fillImage));
            }
            if (shakeTarget != null)
            {
                Vector2 shakeStart = shakeTarget.anchoredPosition;
                builder.Join(LMotion.Shake.Create(shakeStart, new Vector2(shakeStrength, shakeStrength), shakeDuration)
                    .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                    .BindToAnchoredPosition(shakeTarget));
            }

            MotionHandle seq = default;
            seq = builder.Run(cfg =>
            {
                cfg.WithScheduler(MotionScheduler.UpdateIgnoreTimeScale);
                cfg.WithOnComplete(() => _active.Remove(seq));
            });
            _active.Add(seq);
        }
