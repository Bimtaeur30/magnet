...
{
    /// <summary>
    /// VerticalLayoutGroup을 확장해, 플레이 중 레이아웃 변화(위치/크기)를 LitMotion으로
    ...
    /// </summary>
    [AddComponentMenu("Layout/LitMotion Vertical Layout Group")]
    public class TweenVerticalLayoutGroup : VerticalLayoutGroup
    {
        ...
        private readonly Dictionary<RectTransform, CompositeMotionHandle> _childHandles = new();
        ...
        private void ApplyTweens()
        {
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                if (_childHandles.TryGetValue(rt, out CompositeMotionHandle existing))
                    existing.Cancel();
                ...
                handles.Add(LMotion.Create(startPos, targetPos, duration)
                    .WithEase(ease)
                    .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                    .BindToAnchoredPosition(rt));
                handles.Add(LMotion.Create(startSize, targetSize, duration)
                    .WithEase(ease)
                    .WithScheduler(MotionScheduler.UpdateIgnoreTimeScale)
                    .BindToSizeDelta(rt));
                _childHandles[rt] = handles;
            }
        ...
        {
            foreach (CompositeMotionHandle handles in _childHandles.Values)
                handles.Cancel();
            _childHandles.Clear();
            _startPos.Clear();
            _startSize.Clear();
